...
export default function ProductEditPage() {
  ...
  const totalMediaCount = existingUrls.length + newMediaItems.length;
  ...
  const addFiles = (files: FileList | null) => {
    ...
    setNewMediaItems(prev => [...prev, ...items].slice(0, 10 - existingUrls.length));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('매물 수정에 실패했습니다. 다시 시도해주세요.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading) {
    return (
      <section className="create-page">
        <p style={{ textAlign: 'center', color: '#94a3b8', paddingTop: '60px' }}>
          불러오는 중...
        </p>
      </section>
    );
  ...
  return (
    <section className="create-page">
      ...
      <h1 className="create-title">
        <span className="create-title-icon">🦎</span> 매물 수정
      </h1>
            ...
            {newMediaItems.map((m, i) => (
              <div key={`new-${i}`} className="media-preview-item">
                {m.isVideo ? (
                  ...
                  <img src={m.previewUrl} alt={`새 미리보기 ${i + 1}`} />
                )}
